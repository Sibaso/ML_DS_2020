...
def gather_data():
	X,Y=[],[]
	with open("C:\\Users\\pl\\Documents\\Python_Project\\ML_DS_2020\\session_1\\Linear_Regression\\x28.txt",'r') as f:
...
def normalize_and_add_1(X):
	X=np.array(X)
	X_max=np.array([np.amax(X[:,colum_id]) for colum_id in range(X.shape[1])])
	X_min=np.array([np.amin(X[:,colum_id]) for colum_id in range(X.shape[1])])
...
#su dung phuong phap gradient tim ham muc tieu
